...
[X, RESNORM, RESIDUAL, EXITFLAG] = lsqnonlin(@(x) calibration_VG(x, Mkt_mid, S, K, r, T, vega, Nq, OptionType), Init_param, LB, UB, options);

% Get the VG prices with the optimized parameters
...
for idx = 1:length(Mkt_mid)
    ...
    if strcmpi(OptionType{idx}, 'p') && K(idx)/S(idx) < threshold_put
        ...
        VG_Prices(idx) = call_price - S(idx) + K(idx) * exp(-r(idx) * T(idx));
    elseif strcmpi(OptionType{idx}, 'c') && K(idx)/S(idx) > threshold_call
        ...
        VG_Prices(idx) = put_price + S(idx) - K(idx) * exp(-r(idx) * T(idx));
    else
        % Regular pricing
        VG_Prices(idx) = CallPutOptionPriceCOSMthd(cf_vg, OptionType{idx}, S(idx), r(idx), T(idx), K(idx), N, L);
    end
end

% Calculate the differences
Difference_in_Value = Mkt_mid - VG_Prices;
Difference_in_Pct = (Difference_in_Value ./ Mkt_mid) * 100;

% Create and display the results table
results_table = table(K, Mkt_mid, VG_Prices, Difference_in_Value, Difference_in_Pct);
disp(results_table);

RMSE = sqrt(sum(((Mkt_mid - VG_Prices)./(vega)).^2)/Nq);
disp(RMSE);

% VG calibration function
function RMSE = calibration_VG(params, Mkt_mid, S, K, r, T, vega, Nq, OptionType)
    ...
    % Initialize the vector to hold VG prices and errors
    VG_Prices = zeros(length(Mkt_mid), 1);

...
% Optimal VGB parameters
beta = 0.4637;
theta = -0.1113;
eta = 0.0011;   % Optimal eta parameter
sigma = 0.1716;    % Optimal sigma (Brownian component)
...
for date_idx = 1:length(unique_dates)
    ...
    VGB_Prices = zeros(numcontracts, 1);
    VGB_IVs = zeros(numcontracts, 1);
    
    for idx = 1:numcontracts
        cf_vgb = @(u) chfun_VGB(u, r_date(idx), beta, theta, eta, sigma, T_date(idx));
        VGB_Prices(idx) = CallPutOptionPriceCOSMthd(cf_vgb, OptionType_date{idx}, S_date(idx), r_date(idx), T_date(idx), K_date(idx), N, L);
        VGB_IVs(idx) = blkimpv(S_date(idx), K_date(idx), r_date(idx), T_date(idx), VGB_Prices(idx), 'Class', OptionType_date{idx});
    end
    
    % Compute IVMSE for the current date
    IVMSE_values(date_idx) = sum((((Mkt_iv - VGB_IVs)) .^2))/Nq;
end

% Display IVMSE values
...
% Convert unique_dates to numeric format using datenum
...
grid on;
set(gca, 'FontSize', 12);
set(gcf, 'Position', [100, 100, 800, 600]); % Adjust the figure size for better resolution

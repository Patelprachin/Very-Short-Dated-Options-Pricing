...
for bin = 1:num_bins
    ...
    LB = [0.001, 0.001, 0.001, -1, 0.001];  % Lower bounds for the parameters
    UB = [10, 1, 5, 1, 1];  % Upper bounds for the parameters
    ...
    % Calling the optimizer with the objective function
    [X_bin, RESNORM, RESIDUAL, EXITFLAG] = lsqnonlin(@(x) calibration_Heston(x, Mkt_iv_bin, S_bin, K_bin, r_bin, T_bin, OptionType_bin, Nq_bin), Init_param, LB, UB, options);
    ...
    Heston_Prices_bin = zeros(length(Mkt_mid_bin), 1);
    Heston_IVs_bin = zeros(length(Mkt_iv_bin), 1);
    ...
    for idx = 1:length(Mkt_mid_bin)
        cf_heston = @(u) chfun_Heston(u, T_bin(idx), X_bin(1), X_bin(2), X_bin(3), X_bin(4), X_bin(5), r_bin(idx));
        if strcmpi(OptionType_bin{idx}, 'p') && K_bin(idx)/S_bin(idx) < threshold_put
            % Deep OTM put, use call pricing and put-call parity
            call_price = CallPutOptionPriceCOSMthd(cf_heston, 'c', S_bin(idx), r_bin(idx), T_bin(idx), K_bin(idx), N, L);
            Heston_Prices_bin(idx) = max(call_price - S_bin(idx) + K_bin(idx) * exp(-r_bin(idx) * T_bin(idx)), 1e-6);
        elseif strcmpi(OptionType_bin{idx}, 'c') && K_bin(idx)/S_bin(idx) > threshold_call
            % Deep OTM call, use put pricing and put-call parity
            put_price = CallPutOptionPriceCOSMthd(cf_heston, 'p', S_bin(idx), r_bin(idx), T_bin(idx), K_bin(idx), N, L);
            Heston_Prices_bin(idx) = max(put_price + S_bin(idx) - K_bin(idx) * exp(-r_bin(idx) * T_bin(idx)), 1e-6);
        else
            % Regular pricing
            Heston_Prices_bin(idx) = max(CallPutOptionPriceCOSMthd(cf_heston, OptionType_bin{idx}, S_bin(idx), r_bin(idx), T_bin(idx), K_bin(idx), N, L), 1e-6);
        end
    ...
    disp(['Optimal Parameters: kappa = ', num2str(X_bin(1)), ', theta = ', num2str(X_bin(2)), ', gamma = ', num2str(X_bin(3)), ', rho = ', num2str(X_bin(4)), ', v0 = ', num2str(X_bin(5))]);
end

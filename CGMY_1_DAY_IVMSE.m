...
Init_param = [C, G, M, Y];
LB = [0.01 0.01 0.01 0.01];  % Lower bounds for the parameters
UB = [10 10 10 2];  % Upper bounds for the parameters
...
for idx = 1:length(Mkt_mid)
    ...
    cf_CGMY = @(u) chfun_CGMY(u, r(idx), T(idx), X(1), X(2), X(3), X(4), omega);
...
hold on;
plot(moneyness, CGMY_IVs, 'r*-', 'LineWidth', 1.5, 'MarkerSize', 8, 'DisplayName', 'CGMY Model IV');
...
save('cgmy_ivs','CGMY_IVs');

% Objective function for CGMY calibration
function IVMSE = calibration_CGMY(params, Mkt_iv, S, K, r, T, OptionType, Nq)
    ...
    for idx = 1:length(Mkt_iv)
        ...
        cf_CGMY = @(u) chfun_CGMY(u, r(idx), T(idx), C, G, M, Y, omega);
...
function cf = chfun_CGMY(u, r, tau, C, G, M, Y, omega)
    i = complex(0,1);
    varPhi = exp(tau * C * gamma(-Y) * ((M - i*u).^Y - M^Y + (G + i*u).^Y - G^Y));
    cf = varPhi .* exp(i*u*(r + omega)*tau);
end
